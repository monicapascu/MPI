...
int i, j, nx, ny, dims[NDIM], periods[NDIM], reorder=0, coords[NDIM], nvec,crs;
...
{//Intitializare stare
    ...
    MPI_Sendrecv(gblock+nx+3, 1, coltype, left, 345, gblock+2*nx+3, 1, coltype, right, 345, cart_comm, &status);
    ...
    for(i=1;i<ny+1;i++)
        ...
        {
            nvec=0;
            if( *(gblock+(i-1)*(nx+2)+j-1)==0)
                nvec++;
            if(*(gblock+(i-1)*(nx+2)+j)==0)
                nvec++;
            if(*(gblock+(i-1)*(nx+2)+j+1)==0)
                nvec++;
            if(*(gblock+(i)*(nx+2)+j-1)==0)
                nvec++;
            if(*(gblock+(i)*(nx+2)+j+1)==0)
                nvec++;
            if(*(gblock+(i+1)*(nx+2)+j-1)==0)
